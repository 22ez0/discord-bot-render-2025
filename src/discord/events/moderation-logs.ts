...
import { AuditLogEvent, EmbedBuilder, TextChannel, GuildBan, GuildMember, Message, PartialMessage, PermissionsBitField, ChannelType } from "discord.js";
...
async function getLogChannel(guildId: string, client: any) {
    ...
    const fallbackChannel = guild.channels.cache.find((ch: any) => 
...
async function getValidAuditLog(guild: any, type: AuditLogEvent, targetId?: string, maxAge: number = 30000) {
    try {
        ...
        const validLog = auditLogs.entries.find((entry: any) => {
...
createEvent({
    ...
    async run(message: Message | PartialMessage) {
        try {
            ...
            if (!logChannel || logChannel.id === message.channel.id) return; // Evitar loop

            const embed = new EmbedBuilder()
                .setColor(LOG_COLORS.MESSAGE_DELETE)
                .setTitle("🗑️ **MENSAGEM DELETADA**")
                .setDescription(
                    `**Autor:** ${message.author?.tag || "Usuário desconhecido"} \`(${message.author?.id || "ID desconhecido"})\`\n` +
                    `**Canal:** ${message.channel}\n` +
                    `**Conteúdo:** ${message.content?.length ? `\`\`\`${message.content.slice(0, 1000)}${message.content.length > 1000 ? '...' : ''}\`\`\`` : "*Sem conteúdo de texto*"}`
                )
                .setTimestamp()
                .setFooter({ text: `Canal: #${(message.channel as TextChannel).name}` });
...
createEvent({
    ...
    async run(oldMessage: Message | PartialMessage, newMessage: Message | PartialMessage) {
        try {
            if (newMessage.author?.bot) return; // Ignorar bots
            if (!newMessage.guild) return;
            if (oldMessage.content === newMessage.content) return; // Ignorar edições sem mudança de conteúdo
            ...
            const embed = new EmbedBuilder()
                ...
                .setTimestamp()
                .setFooter({ text: `Canal: #${(newMessage.channel as TextChannel).name}` });
        ...
        try {
            ...
            if (!oldTimeout && newTimeout) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(
                        `**Usuário:** ${newMember.user.tag} \`(${newMember.user.id})\`\n` +
                        `**Moderador:** ${executor ? executor.tag : "Sistema"}\n` +
