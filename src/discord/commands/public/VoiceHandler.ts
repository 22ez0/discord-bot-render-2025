import { Client, ActivityType, VoiceChannel, VoiceState } from "discord.js";
 ...
 */
export async function joinVoiceChannelById(guildId: string, channelId: string, client: Client): Promise<boolean> {
  try {
 ...
 */
export async function autoConnectVoiceChannels(client: Client) {
  ...
  try {
    for (const guild of client.guilds.cache.values()) {
      ...
      if (config.automation?.voiceChannels?.enabled && config.automation.voiceChannels.joinChannelId) {
        const success = await joinVoiceChannelById(
          guild.id, 
          config.automation.voiceChannels.joinChannelId, 
          client
        );
