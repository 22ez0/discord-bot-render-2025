...
async function handleAntiSpam(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`**Status:** ${isEnabled ? '🟢 Ativado' : '🔴 Desativado'}\n\n` +
    ...
    const row = new ActionRowBuilder()
        ...
        .setEmoji(isEnabled ? '🔴' : '🟢'), new ButtonBuilder()
...
async function handleAntiLinks(interaction) {
    const guildId = interaction.guild.id;
    const config = storage.getServerConfig(guildId);
...
async function handleAutoRoles(interaction) {
    const guildId = interaction.guild.id;
    const config = storage.getServerConfig(guildId);
...
async function handleBannedWords(interaction) {
    const guildId = interaction.guild.id;
    const config = storage.getServerConfig(guildId);
...
async function handleAntiSpamAction(interaction, action) {
    const guildId = interaction.guild.id;
    const config = storage.getServerConfig(guildId);
    switch (action) {
        case 'enable':
            const antiSpamUpdate = {
                enabled: true,
                antiSpam: { enabled: true, maxMessages: 5, timeWindow: 10, punishment: 'timeout', punishmentDuration: 10 }
            };
            ...
            break;
        case 'disable':
            const currentAntiSpam = config.automod?.antiSpam || { maxMessages: 5, timeWindow: 10, punishment: 'timeout', punishmentDuration: 10 };
...
async function handleAntiLinksAction(interaction, action) {
    const guildId = interaction.guild.id;
    const config = storage.getServerConfig(guildId);
    switch (action) {
        case 'enable':
...
async function handleBannedWordsAction(interaction, action) {
    const guildId = interaction.guild.id;
    const config = storage.getServerConfig(guildId);
    switch (action) {
        case 'enable':
...
async function handleWelcomeAction(interaction, action) {
    const guildId = interaction.guild.id;
    const config = storage.getServerConfig(guildId);
    switch (action) {
        case 'enable':
...
async function handleAutoRolesAction(interaction, action) {
    const guildId = interaction.guild.id;
    const config = storage.getServerConfig(guildId);
    switch (action) {
        case 'enable':
...
async function showAntiSpamConfigModal(interaction) {
    const guildId = interaction.guild.id;
    const config = storage.getServerConfig(guildId);
    ...
    const timeWindowInput = new TextInputBuilder()
        ...
        .setRequired(true);
    const row1 = new ActionRowBuilder().addComponents(maxMessagesInput);
    const row2 = new ActionRowBuilder().addComponents(timeWindowInput);
