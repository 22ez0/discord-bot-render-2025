...
async function handleClearFast(interaction) {
...
async function handleClearUser(interaction) {
    ...
    const quantityInput = new TextInputBuilder()
        ...
        .setMaxLength(3)
        .setRequired(true);
    modal.addComponents(new ActionRowBuilder().addComponents(userInput), new ActionRowBuilder().addComponents(quantityInput));
    await interaction.showModal(modal);
}
async function handleClearCustom(interaction) {
    const modal = new ModalBuilder()
    ...
    const filterInput = new TextInputBuilder()
        ...
        .setRequired(false);
    modal.addComponents(new ActionRowBuilder().addComponents(quantityInput), new ActionRowBuilder().addComponents(filterInput));
    await interaction.showModal(modal);
}
