...
import { ApplicationCommandType, ApplicationCommandOptionType, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, PermissionsBitField } from "discord.js";
...
async function handleEmojiMenu(interaction) {
    await interaction.deferReply({ flags: 64 });
    const embed = new EmbedBuilder()
        .setColor("#9c88ff")
        .setTitle("🎭 **GERENCIAR EMOJIS**")
        .setDescription(`> **Escolha uma ação para gerenciar os emojis do bot:**\n\n` +
...
async function handleEditEmojis(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`> **Selecione uma categoria para personalizar:**\n\n` +
...
async function handleResetEmojis(interaction) {
    ...
    if (success) {
        const embed = new EmbedBuilder()
            ...
            .setDescription(`✅ **Todos os emojis foram resetados para o padrão!**\n\n` +
...
async function handleEditTexts(interaction) {
    await interaction.deferReply({ flags: 64 });
    const embed = new EmbedBuilder()
        .setColor("#9c88ff")
        .setTitle("📝 **EDITAR TEXTOS DOS PAINÉIS**")
        .setDescription(`> **Selecione qual painel você deseja personalizar:**\n\n` +
...
async function handleCustomizeColors(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`> **Personalize as cores dos embeds de cada painel:**\n\n` +
...
async function handleViewConfig(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`> **Visão geral das personalizações aplicadas:**\n\n` +
...
async function handleResetAll(interaction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`⚠️ **ATENÇÃO: Esta ação irá resetar TODAS as personalizações!**\n\n` +
        `**O que será resetado:**\n` +
...
export function updateServerEmoji(guildId, emojiKey, emojiValue) {
    const customEmojis = storage.getServerEmojis(guildId) || {};
