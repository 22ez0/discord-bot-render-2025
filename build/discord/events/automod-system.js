...
const identicalTracker = new Map();
...
async function checkAntiSpam(message, antiSpamConfig) {
...
function checkAntiLinks(message, antiLinksConfig) {
...
function checkBannedWords(message, bannedWordsConfig, member) {
    const content = message.content.toLowerCase();
    // Verificar se o usuário tem roles que ignoram o filtro
    if (bannedWordsConfig.ignoreRoles?.some((roleId) => member.roles.cache.has(roleId))) {
        return false;
    }
...
async function handleViolation(message, violationType, punishment) {
    try {
        // Deletar a mensagem
...
async function sendWarning(member, violationType) {
    try {
        const embed = new EmbedBuilder()
            .setColor("#ff9f43")
            .setTitle("⚠️ **AVISO** | Auto-Moderação")
            .setDescription(`Você recebeu um aviso por violação das regras:\n\n` +
            `**Tipo:** ${violationType}\n` +
...
async function timeoutMember(member, violationType) {
    try {
...
async function kickMember(member, violationType) {
    try {
...
async function banMember(member, violationType) {
    try {
...
async function checkAdvancedAntiSpam(message, config) {
    const userId = message.author.id;
    const now = Date.now();
...
function checkCustomAntiLinks(message, config) {
    const content = message.content.toLowerCase();
    // Verificar convites do Discord
    const discordInviteRegex = /(https?:\/\/)?(www\.)?(discord\.(gg|io|me|li)|discordapp\.com\/invite)\/.+/gi;
...
function checkCustomBannedWords(message, config, _member) {
    const content = message.content.toLowerCase();
...
async function handleAdvancedViolation(message, violationType, punishment, duration) {
    try {
        // Deletar a mensagem
        await message.delete().catch(() => null);
        const member = message.member;
...
async function handleWarningSystem(member, violationType) {
...
function checkParceiroPermission(message, member) {
    const guildId = message.guild?.id;
    if (!guildId)
        return false;
...
async function handleCLCommand(message, config) {
    try {
        const userId = message.author.id;
        const member = message.member;
        if (!member)
        ...
        while (checkedCount < maxChecked && deletedCount < 100) {
            ...
            if (userMessages.size > 0) {
                try {
                    ...
                    if (userMessages.size > 1 && userMessages.every((msg) => (Date.now() - msg.createdTimestamp) < 14 * 24 * 60 * 60 * 1000)) {
        ...
        // Confirmar limpeza
        const confirmMessage = await message.channel.send(`🧹 **Limpeza concluída!** ${deletedCount} mensagem(ns) de ${member.user.username} foram removidas.`).catch(() => null);
