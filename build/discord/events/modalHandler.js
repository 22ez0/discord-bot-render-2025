...
async function processBanModal(interaction) {
...
async function processKickModal(interaction) {
...
async function processTimeoutModal(interaction) {
...
async function processClearModal(interaction) {
...
async function processSpamLimitsModal(interaction) {
...
async function processAddLinkModal(interaction) {
...
async function processWelcomeMessageModal(interaction) {
...
async function process8BallModal(interaction) {
...
async function processAntiSpamConfigModal(interaction) {
...
async function processFwckClearFastModal(interaction) {
    ...
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`**${messagesToDelete.length} mensagens** foram deletadas com sucesso.\n` +
            `**Moderador:** ${interaction.user.tag}\n` +
...
async function processFwckClearUserModal(interaction) {
    ...
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`**${userMessages.size} mensagens** de ${targetUser.tag} foram deletadas.\n` +
            `**Moderador:** ${interaction.user.tag}\n` +
            `**Canal:** ${interaction.channel}`)
...
async function processFwckClearCustomModal(interaction) {
    const quantity = interaction.fields.getTextInputValue('quantity');
    ...
    try {
        ...
        if (filterWord && filterWord.trim()) {
            ...
            messagesToDelete = messagesToDelete.filter(msg => msg.content.toLowerCase().includes(filter));
        }
