import { Client, GatewayIntentBits, ActivityType, REST, Routes, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, PermissionFlagsBits } from 'discord.js';
...
function createMainPanel(guildName) {
    const embed = new EmbedBuilder()
        ...
        .setColor(0x5865F2)
        .addFields({ name: '🧹 Sistema CL', value: 'Configure comandos de limpeza personalizados', inline: true }, { name: '📋 Logs', value: 'Configure logs detalhados para eventos', inline: true }, { name: '🛡️ AutoMod', value: 'Configure auto-moderação avançada', inline: true }, { name: '👋 Boas-vindas', value: 'Configure mensagens de boas-vindas', inline: true }, { name: '🎭 Auto-Role', value: 'Configure cargos automáticos', inline: true }, { name: '⚙️ Configurações', value: 'Configurações gerais do servidor', inline: true })
...
function createCLPanel(config, guildName) {
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: '📊 Status Atual', value: config.clSystem.enabled ? '🟢 **Ativado**' : '🔴 **Desativado**', inline: true }, { name: '🗑️ Apagar comando', value: config.clSystem.deleteOwnMessage ? '✅ Sim' : '❌ Não', inline: true }, { name: '👥 Usuários permitidos', value: config.clSystem.allowedUsers.length > 0 ? `${config.clSystem.allowedUsers.length} usuários` : 'Nenhum', inline: true }, { name: '🎭 Cargos permitidos', value: config.clSystem.allowedRoles.length > 0 ? `${config.clSystem.allowedRoles.length} cargos` : 'Todos', inline: true }, { name: '⚡ Comandos personalizados', value: config.clSystem.customTriggers.length > 0 ? config.clSystem.customTriggers.join(', ') : 'Apenas "cl"', inline: true }, { name: '\u200b', value: '\u200b', inline: true })
        .setFooter({ text: `Servidor: ${guildName}` })
        .setTimestamp();
    const row = new ActionRowBuilder()
        .addComponents(new ButtonBuilder()
        ...
        .setStyle(config.clSystem.enabled ? ButtonStyle.Danger : ButtonStyle.Success), new ButtonBuilder()
...
function createLogsPanel(config, guildName) {
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: '📊 Status', value: config.logging.enabled ? '🟢 **Ativado**' : '🔴 **Desativado**', inline: true }, { name: '📝 Canal de logs', value: config.logging.channelId ? `<#${config.logging.channelId}>` : '❌ Não configurado', inline: true }, { name: '📈 Eventos ativos', value: Object.values(config.logging.events).filter(Boolean).length + '/12', inline: true })
        .setFooter({ text: `Servidor: ${guildName}` })
        .setTimestamp();
    const row = new ActionRowBuilder()
        .addComponents(new ButtonBuilder()
        ...
        .setStyle(config.logging.enabled ? ButtonStyle.Danger : ButtonStyle.Success), new ButtonBuilder()
...
function createAutoModPanel(config, guildName) {
    const embed = new EmbedBuilder()
        ...
        .setColor(0x5865F2)
        .addFields({ name: '🔗 Anti-Links', value: config.automod.antiLink.enabled ? '✅ Ativo' : '❌ Inativo', inline: true }, { name: '🚫 Filtro de Palavras', value: config.automod.wordFilter.enabled ? '✅ Ativo' : '❌ Inativo', inline: true }, { name: '📢 Anti-Spam', value: config.automod.antiSpam.enabled ? '✅ Ativo' : '❌ Inativo', inline: true }, { name: '🔠 Anti-Caps', value: config.automod.antiCaps.enabled ? '✅ Ativo' : '❌ Inativo', inline: true }, { name: '✅ Links permitidos', value: config.automod.antiLink.allowedDomains.length.toString(), inline: true }, { name: '🚫 Palavras banidas', value: config.automod.wordFilter.bannedWords.length.toString(), inline: true })
        .setFooter({ text: `Servidor: ${guildName}` })
        .setTimestamp();
    const row = new ActionRowBuilder()
        .addComponents(new ButtonBuilder()
        ...
        .setLabel('🔗 Anti-Links')
        .setStyle(config.automod.antiLink.enabled ? ButtonStyle.Success : ButtonStyle.Secondary), new ButtonBuilder()
        .setCustomId('automod_wordfilter')
        .setLabel('🚫 Palavras')
        .setStyle(config.automod.wordFilter.enabled ? ButtonStyle.Success : ButtonStyle.Secondary), new ButtonBuilder()
    ...
    const row2 = new ActionRowBuilder()
        ...
        .setLabel('🔠 Anti-Caps')
        .setStyle(config.automod.antiCaps.enabled ? ButtonStyle.Success : ButtonStyle.Secondary), new ButtonBuilder()
        .setCustomId('back_main')
        .setLabel('◀ Voltar')
        .setStyle(ButtonStyle.Secondary));
    return { embeds: [embed], components: [row, row2] };
...
function createWelcomePanel(config, guildName) {
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: '📊 Status', value: config.welcome.enabled ? '🟢 **Ativado**' : '🔴 **Desativado**', inline: true }, { name: '📝 Canal', value: config.welcome.channelId ? `<#${config.welcome.channelId}>` : '❌ Não configurado', inline: true }, { name: '💬 Mensagem', value: config.welcome.message ? '✅ Configurada' : '❌ Padrão', inline: true })
        .setFooter({ text: `Servidor: ${guildName}` })
        .setTimestamp();
    const row = new ActionRowBuilder()
        .addComponents(new ButtonBuilder()
        ...
        .setStyle(config.welcome.enabled ? ButtonStyle.Danger : ButtonStyle.Success), new ButtonBuilder()
...
function createAutoRolePanel(config, guildName) {
    const embed = new EmbedBuilder()
        ...
        .addFields({ name: '📊 Status', value: config.autoRole.enabled ? '🟢 **Ativado**' : '🔴 **Desativado**', inline: true }, { name: '🎭 Cargo', value: config.autoRole.roleId ? `<@&${config.autoRole.roleId}>` : '❌ Não configurado', inline: true }, { name: '📝 Canal', value: config.autoRole.channelId ? `<#${config.autoRole.channelId}>` : '❌ Não configurado', inline: true })
        .setFooter({ text: `Servidor: ${guildName}` })
        .setTimestamp();
    const row = new ActionRowBuilder()
        .addComponents(new ButtonBuilder()
        ...
        .setStyle(config.autoRole.enabled ? ButtonStyle.Danger : ButtonStyle.Success), new ButtonBuilder()
...
client.on('interactionCreate', async (interaction) => {
        ...
        try {
            if (interaction.customId === 'panel_cl') {
                ...
                const selectMenu = new StringSelectMenuBuilder()
                    ...
                    .addOptions(config.clSystem.allowedUsers.slice(0, 25).map(userId => new StringSelectMenuOptionBuilder()
                ...
                const triggerInput = new TextInputBuilder()
                    ...
                    .setRequired(true)
                    .setMaxLength(20);
                const actionRow = new ActionRowBuilder().addComponents(triggerInput);
                modal.addComponents(actionRow);
                await interaction.showModal(modal);
            }
    ...
    if (interaction.isStringSelectMenu()) {
        ...
        try {
            if (interaction.customId === 'cl_select_add_roles') {
                ...
                config.clSystem.allowedRoles = config.clSystem.allowedRoles.filter(roleId => !interaction.values.includes(roleId));
                saveServerConfig(interaction.guild.id, config);
                ...
                config.clSystem.allowedUsers = config.clSystem.allowedUsers.filter(userId => !interaction.values.includes(userId));
                saveServerConfig(interaction.guild.id, config);
                await interaction.update({
    ...
    async function analyzeUserForAlt(user, guild) {
        const factors = [];
        let score = 0;
    ...
    async function listServerEmojis(guild) {
        if (!guild)
        ...
        if (totalEmojis > 0) {
            const emojiList = emojis.first(20).map(emoji => `${emoji} \`:${emoji.name}:\``).join(' ');
    ...
    async function getEmojiStats(guild) {
        if (!guild)
            throw new Error('Guild não encontrada');
        const emojis = guild.emojis.cache;
    ...
    async function detectAllEmojis(text) {
        const results = {
            customEmojis: [],
            animatedEmojis: [],
            unicodeEmojis: []
